...
one of many microscope techniques to circumvent \textsc{Abbe’s}
...
\begin{figure}[h]
  \centering \includegraphics[width=0.5\textwidth]{jablonski_2.png}
  \caption{asdf, \cite{asdf}.}
  \label{f:asdf}
\end{figure}

Both processes happen by chance, which generally is viewed as a
probability within quantum mechanics. This leads to the stochastic
variables of the \x{rate} $\rho$ that we can measure in experiments as
occurrence per time, and the \x{lifetime} $\tau$ of a given state as
...
While in the triplet state, the molecule can not absorb a photon to
get \e{back} to the excited state $S_1$, staying dark until it
falls back to ground state $S_0$.

Some chemical reaction like oxidation might change the molecules
characteristics so it loses the ability to fluorescence for good,
which is called \e{\x{photobleaching}}. Some other reaction may
re-enable the fluorescence, that is one of the reasons for the use of
oxygen-depleted \x{buffers} in \gls{smlm}.
...
would exceed the scope of this paper. The interested reader is
referred to /t{ref}.
...
like a \gls{emccd}.
%(an alternative would be a \gls{scmos}).
...
resolution; the basic principle of all \gls{smlm} techniques is to
separate the signal of neighbouring individual emitters in \e{time},
which allows to determine their positions with nanometer precision.

There are many similar methods like \gls{storm}, which was invented
simultaneously by different research groups, and has since the mid
2000s become the most widely used technique for \gls{smlm} \t{ref}.

First, all fluorophores are put to their triplet state with intense
laser light of appropriate frequency, in our case . In our experimental setup this means the live image gets
...
\e{off} states. This somewhat static image is recorded, think of it as
a snapshot of the night sky with a shutter speed of 1/50~s. This
process has to be repeated many times, until all the fluorophores have
been \e{on} several times stochastically. In our case this yields
...
37 (3rd order \x{spherical aberration}) plus the 4th order spherical
...
The principle of \gls{storm} is based on an appropriate \e{\x{storm
buffer}}, which suppresses the activation of the \e{\x{bright state}},
...
\x{excitation wavelengths}, in our case at \SI{645}{\nm} (red)
respective at \SI{488}{\nm} (blue). After its central ingredients
\x{glucose} and \x{glucose oxidase}, the buffer we used throughout this paper
for all single channel measurements unless noted otherwise is called
\acrshort{gloxy} buffer\index{Gloxy buffer}.
...
used is based on \cite{nab16}, and called \acrshort{oxea}\index{OxEA
  buffer} after its main ingredients \x{OxyFlour} and
\gls{mea}\index{mercaptoethylamine hydrochloride}:
...
\subsection{Cram\'er Rao Lower Bound}\index{Cram\'er Rao lower bound}
...
The prepared samples, both beads and \gls{npc}, labeled with both
\e{\x{Alexa Fluors}} \x{AF647} and \x{AF488} in \gls{gloxy} buffer,
described in detail in Section~\ref{s:m:gloxy}, which works well with
either red or blue excitation.\\
...
Section~\ref{s:m:oxea}, which is needed if one wants to use both red
and blue excitation.\\
...
\begin{table}[!htb]
  \caption{Ingredients used for preparation of OxEA buffer for dual
    colour \gls{dstorm} buffer.} \label{t:oxea}
...
\t{you could add a scheme of the NPC so that it is clear that what we
  see in this pictures resembles the real 8-fold structure of the
  pore}
...
\t{now the scheme would be really helpful}
...
% .   __|  _ \  __ \   __| | |   |  __| |  _ \  __ \  
% .  (    (   | |   | (    | |   |\__ \ | (   | |   | 
% . \___|\___/ _|  _|\___|_|\__,_|____/_|\___/ _|  _| 
% .
\chapter{Conclusion}

\t{I would fuse this with the above discussion or avoid making the
  subchapters here}
...
regime between about 0 and \SI{250}{\nano\meter}.
