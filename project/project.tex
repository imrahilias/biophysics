...
\begin{titlepage}
  ...
  \vspace{1cm}
  {\Huge \textsc{Three Dimensional Dual Colour Single Molecule Localisation Microscopy}}\\
  \vspace{1cm}
  concluded at the\\
  \vspace{.5cm}
...
dimensional \gls{smlm}, by alternating excitation with different
lasers, but still measuring only one channel.\\

\gls{smlm} is a technique of circumvent \textsc{Abbe’s} diffraction
limit, enabling the study of structures smaller than half the
wavelength of light; thus reaching super resolution fluorescence
microscopy.


\section{Fluorescence Microscopy}

Fluorescence microscopy is a optical microscopy technique using
...
fluorophores; causing them to emit (much weaker) light on a slightly
longer wavelength. After separation from excitation light using
spectral filters, this image is captured by a sensitive \gls{scmos}
camera.
...
only illuminating the bound fluorophores, \gls{tirf} was
invented. This is a type of microscope with which a thin region of a
...
\section{Single Molecule's Location}
...
precise enough to fit a Gaussian to each of the diffuse dots in the
frame, in order to estimate their centre---thus, the position of the
fluorophore. For this thesis we used the ImageJ plugin
\tt{ThunderSTORM} \t{cit} for this task.
...
displacement (c) or overcounting (d), \t{cit}.
...
Fitting a full Point Spread Functions (PSF)---using prior calculated
\textsc{Zernike} Modes---to such a stack of images containing reasonably spaced
fluorescence signals even yields a list of 3d localisations.
...
For the Analysis of \gls{dstorm} fluorescence microscopy, \gls{mle} is
widely accepted as a gold standard; We closely follow the method from
\t{cit} here. First we assume a \textsc{Poissonian} probability amount
of photons $n_k$ detected in pixel $k$:

\begin{equation}
  P_k(n_k | \mu_{\symbf{\theta},k}) = \n{e}^{-\mu_{\symbf{\theta},k}} \frac{\mu_{\symbf{\theta},k}^{n_k}}{n_k!}
\end{equation}

where the expectation values $\mu_{\symbf{\theta},k}$ is defined as:

\begin{equation}
\mu_{\symbf{\theta},k} = \beta +  s *h_k( x_0, y_0, z_0 )
\end{equation}

depending on the form of the \gls{psf} model $h$ and on the parameter
vector $\symbf{\theta}$:
...
estimated by finding a minimum of the total negative log-likelihood
function using the MATLAB tool \tt{fminunc}:
...
In first order approximation the \gls{psf} can be aberration-free, but
for \gls{smlm} this is not going to be precise enough. In this thesis
we employ a \gls{psf} model expressed in a \textsc{Zernike} modes
basis using \textsc{Noll} coefficients \t{cit} ranging from 2 to 37
(3rd order spherical aberration) plus the 4th order spherical mode
$Z_{56}$:
...
We used dSTORM microscopy throughout this thesis, which is the
\e{direct} variant of STORM, utilising very bright fluorophores, with
a high rate of photoswitching and minimal photobleaching.
...
The principle of \gls{storm} is based on an appropriate \e{storm
  buffer}, which suppresses the activation of the \e{bright state},
and limits the amount of oxygen, the main source of photobleaching.

Single channel \gls{smlm} may employ the same buffer for both
excitation wavelengths, in our case at \SI{645}{\nm} (red) respective
at \SI{488}{\nm} (blue). After its central ingredients glucose and
oxidase, the buffer we used throughout this paper for all single
channel measurements unless noted otherwise is called \acrshort{gloxy}
buffer.
...
\item with \gls{pbs} adjust to pH 7.6 .
\end{itemize}

\subsection{OxEA Buffer}\label{s:m:oxea}

Dual colour fluorescence microscopy poses novel challenges to find a
proper \gls{dstorm} buffer, that works for both excitation wavelengths
simultaneously---thus two distinct fluorophores AF647 and
AF488---without interfering with each other. The buffer composition we
used is based on \t{cit}, and called \acrshort{oxea} after its main
ingredients OxyFlour and \gls{mea}:

\subsubsection{OxEA buffer concentration}
\begin{itemize}
\item 50 \si{\milli\mol} \gls{mea} (Sigma-Aldrich).
\item 3~vol\% OxyFlourTM (Oxyrase Inc., Mansfield, Ohio, U.S.A.).
\item 20~vol\% of \todo{60\%} sodium DL-lactate syrup (L1375, Sigma-Aldrich).
...
\begin{table}[!htb]
  \caption{Ingredients used for preparation of OxEA buffer for dual
    channel \gls{dstorm} buffer.} \label{t:oxea}
...
red and blue laser light, with the Phase Retrieval program by
[Jesacher et al 21?]; each on a stack of 50000~\gls{dstorm} images of
\SI{100}{\nm} \t{100nm?} beads stained with fluorescence dyes for both
red (\SI{645}{\nm}) and blue (\SI{488}{\nm}) laser
light.\\
...
To further complicate things, the new Olympus \SI{1.5}{\NA} objective
comes with a Correction Collar to compensate for aberrations---which
naturally vary slightly for both color channels. In order to find the
best Correction Collar setting of the Olympus \SI{1.5}{\NA} objective
for \gls{smlm}, the Point Spread Functions (PSF) is computed for each
of the three settings of the correction collar $\{0.13,0.17,0.19\}$;
using the program by \t{Jesacher et al}, described in
Section~\ref{s:m:psf}.\\
...
via phase retrieval \t{Jesacher et al}.\\
...
identical arbitrary, but consistent signal strength (2500) respective
background (100).\\


\section{Dual Colour Projection}

The ultimate goal of this thesis is to facilitate dual colour
\gls{smlm}, by alternating excitation with different lasers, but still
measuring only one channel. The alternation ensures the comparability
of the two sets of images; so that they are affected by drift over
time, bleaching, etc. in the same way.
...
ultimately want to overlay them. We hereby propose a calibration step,
based on evaluating evenly dyed beads in both channels, analysing the
localisations, and finally figuring out their offset.


\subsection{Rigid Transform}\label{s:m:rig}

In first order approximation, this offset may be thought as linear, so
the two sets are related via a \e{rigid} transform; thus only by
...
ways, using least squares and \gls{svd} based on \t{cit}.

In this short overview, the Lines refer to the the function \tt{rig()}
shown in Section~\ref{s:c:rig}, where the full code is
listed. Basically, one first calculates the centroids of the two sets
of points \tt{p},\tt{q}, and shifts both sets to their respective
...
the set p to the set q is found via SVD (block 2 at Lines~30--51). As
a last step the translation between set p and q is computed using the
known rotation (block 3 at line 53).


\subsection{Affine Transform}\label{s:m:affine}

The next more complex approach includes
other---nonlinear---transformations like \e{shearing} or \e{scaling},
called an \e{affine} transform. Our approach is based on \t{cit}, and
can be viewed as a more complex variant of the rigid transform shown
in Section~\ref{s:m:rig}, still using least squares and \acrfull{svd} to
find the optimal affine transform in 3-dimensional euclidean space.

Between centering and SVD, which follow the rigid transform, a
\e{orthogonal reduction} is performed (block 2 at Lines~37--60). Here
the covariance matrices of both sets \tt{p} and \tt{q} are computed,
to form their inverse via a Choleski decomposition (Lines~44--49). The
...
Section~\ref{s:c:sim}, as well as in my Git repository at
\t{git}. First, each simulation creates a cylinder-shaped
pseudo-random point cloud \tt{p}, and deducts some projection to get a
second set \tt{q}, related to \tt{p} by either a rigid or an affine
transformation.
...
several real dual colour \gls{smlm} data sets of \gls{npc}s of
two-color in-focus measurements of different cells successively
recorded in the same sample. The fluorescence images are analysed with
\e{Thunderstorm}, a \gls{dstorm} plugin for ImageJ, to obtain 2d
localisations based on Gaussian fits.
...
because the singular-value-decomposition can faults on 2d data.
...
to sort the imported data file into the two alternating color
channels. The code is listed in Section~\ref{s:c:cockpit}, and
Section~\ref{s:c:ampel}, as well as in my Git repository at \t{git}.
...
light; the buffer constituents play a central role.\\

The prepared samples, both beads and \gls{npc}s have been evaluated
using \gls{gloxy} buffer, described in detail in
Section~\ref{s:m:gloxy}, which works well with either red or blue
excitation.\\

Additionally the samples, both beads and \gls{npc}s have been
evaluated using \gls{oxea} buffer, described in detail in
Section~\ref{s:m:oxea}, which is needed if one wants to use both red and
blue excitation.

The \gls{oxea} buffer did work, but offered notably less blinking, as
well as was much lower \gls{snr} For the red channel compared to using
\gls{gloxy} buffer; with the blue channel being worse still.
...
\clearpage\subsection{Cram\'er Rao Lower Bound}

The Estimated \acrfull{crlb} for different correction Collar settings
(variing linestyles for 0.13, 0.17, 0.19) of the Olympus \SI{1.5}{\NA}
objective; for X, Y and Z axis (top, middle, and bottom); for both red
and blue channel (colors) is shown in
Figure~\ref{f:all}.\\
...
\begin{figure*}[!t]
  ...
  \caption{Estimated \textsc{Cram\'er Rao} lower bound for different correction
    ...
    and bottom); for both red and blue channel (colors).}
  \label{f:all}
\end{figure*}
~ % dirty hack
...
\begin{figure}[!t]
  ...
  \caption{ Estimated \textsc{Cram\'er Rao} lower bound for different correction
    ...
    Olympus \SI{1.5}{\NA} objective; grouped by X, Y and Z axis (top,
    middle, and bottom).}
  \label{f:axes}
\end{figure}
% 
...
In-focus, one colour, two dimensional \gls{smlm} is performed on all
the samples, and evaluated by \tt{Thunderstorm} via ImageJ. This gives
a good x,y precision, an ideal reference for the later three
dimensional localisation, whose x,y precision will most likely be much
lower.
...
visible, even the more or less eight-fold symmetry of the nucleoporins
is resolved.
...
defocused for \SI{500}{\n\m}, only those \tt{z} values between 0 and
...
belonging to a cluster), color coded by photon count.
...
Figure~\label{f:8_best_filtered_clusters} and
Figure~\label{f:9_best_filtered_clusters}shows a 3d plot of the
...
\begin{figure}[h!]
  ...
  \caption{3d plot of the localisations within the 10 \e{best}
    clusters, color coded by cluster assignment.}
...
In order to perform two-colour \gls{smlm} we marked \gls{npc} and bead
samples with an equal amount of both AF488 and AF647 fluorescence
dyes.

A first measurement using the Gloxy buffer worked well for the red
channel (for which the Gloxy buffer is designed) but did not lead to a
good SNR in the blue channel.

Using the OxEA buffer described in Section~\t{sec}, we got both
channels working ok, but the overall SNR proved to be not good enough
for drift analysis via ImageJ, or even particle tracking. There is no
way we could try to recover the 3d transformation relating those data
sets under this noisy conditions.
...
two-dimensional Thunderstorm \gls{smlm} data for a second proof of work.
...
the twelve analysed sets, with mean and standard derivation of:
...
The estimated \textsc{Zernike} modes of the PSF are mostly plausible; based on
...
\section{3d SMLM Analysis: NPC}~\label{s:d:ananpc}
...
\item [quality] The definitions of the two quality entities
  ...
  as fitting the histogram in z direction to a Gaussian.
...
Some of the results of the \gls{npc} analysis might still prove themselves
...
\item [high variance] One might want to exclude clusters with an
  overly high variance, as these might be in fact two \gls{npc}s too close
...
\item [spread] Likewise we may exclude clusters spreading far in z,
  due to our knowledge of the \gls{npc} heigth, as well as in x,z due to the
  \gls{npc}s well defined radius.
\end{description}
...
Simply put, the algorithm \tt{rig()} finds the \e{best}
rigid transformation, whatever the real relation is. This poses a
serious caveat for experimental data, since one is usually not in
possession of any form of ground truth to check whether the result is
plausible or not.
...
$\b{t}$ and rotation matrices $\b{R}$ in Section~\ref{s:r:dcsim} enables
a deeper understanding of the involved misconceptions---that
...
dangerous caveat for the dual colour \gls{smlm} analysis, where we cannot
rule out rotations.
...
As a second proof of work, we tested both rigid and affine recovery on
several real dual colour \gls{smlm} data sets of two color in focus
...
\subsection{Simulate Dual Chanel 3d SMLM Data: simulate.m}\label{s:c:sim}
\begin{minted}{octave}
#!/bin/octave
## @ moritz siegel
