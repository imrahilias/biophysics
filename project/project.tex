...
\begin{titlepage}
  ...
  \vspace{1cm}
  {\Huge \textsc{Three Dimensional Dual Channel Single Molecule Localisation Microscopy}}\\
  \vspace{1cm}
  concluded at the\\
  \vspace{.5cm}
...
\t{we want two color dyed \gls{smlm} data in sync, how to get there?}

Single Molecule Localisation Microscopy (\gls{smlm}) is a technique of
...
one knows the shape of a point source in varying degrees of de-focus,
...
\t{cit} here. First we assume a Poissonian probability amount of
photons $n_k$ detected in pixel $k$:
...
we employ a \gls{psf} model expressed in a Zernike modes basis using
Noll coefficients \t{cit} ranging from 2 to 37 (3rd order spherical
aberration) plus the 4th order spherical mode $Z_{56}$:
...
Single channel \gls{smlm} may employ the same buffer for both excitation
...
\begin{itemize}
\item 50 \si{\milli\mol} $\beta$-MercaptoEthylamine hydrochloride (\gls{mea}, Sigma-Aldrich).
...
\item in \gls{pbs}, pH 7.6 .
\end{itemize}

\subsection{OxEA Buffer}

Dual channel Fluorescence microscopy poses novel challenges to find a
proper \gls{dstorm} buffer, that works for both excitation
wavelengths---thus two distinct fluorophores AF647 and AF488. The
buffer composition we used is based on \t{cit}, and called OxEA after
its main ingredients OxyFlour and \gls{mea}:
...
\item in \gls{pbs}, pH adjusted to 8–8.5 with NaOH.
\end{itemize}
...
program by [Jesacher et al 21?]; each on a stack of 50000~\gls{dstorm}
images of \SI{100}{\nm} \t{100nm?} beads stained with fluorescence
dyes for both red (\SI{645}{\nm}) and blue (\SI{488}{\nm}) laser
light.\\
...
for \gls{smlm}, the Point Spread Functions (PSF) is computed for each of the
three settings of the correction collar $\{0.13,0.17,0.19\}$; using
the program by [Jesacher et al 21?].\\


\subsection{Cramer Rao Lower Bound}

In order to find the best Correction Collar setting of the Olympus
\SI{1.5}{\NA} objective for \gls{smlm}, the Cramer Rao Lower Bound (\gls{crlb}) is
...
All \gls{crlb}s are computed at a defocus position of -500~nm, in steps of
5~nm from 0 to 500 nm. For all Estimations we assume identical
arbitrary signal strength (2500) respective background (100).\\


\section{Dual Channel SMLM}

The ultimate goal of this thesis is to facilitate dual channel \gls{smlm},
by alternating excitation with different lasers. The alternation
ensures the comparability of the two sets of images; so that they are
affected by drift over time, bleaching, etc. in the same way.
...
\subsection{Rigid Transform}\label{m:rig}
...
ways, using least squares and single-value-decomposition (SVD) based
on \t{cit}.  In this quick overview, the Lines refer to the the
function \tt{rig()} shown in Section~\ref{a:rig}, where the full code
is listed. Basically, one first calculates the centroids of the two
...
\subsection{Affine Transform}\label{m:affine}
...
in Section~\ref{r:rig}, still using least squares and
single-value-decomposition (SVD) to find the optimal affine transform
in 3-dimensional euclidean space. Between centering and SVD, which
follow the rigid transform, a \e{orthogonal reduction} is performed
(block 2 Lines~37--60). Here the covariance matrices of both sets
\tt{p} and \tt{q} are computed, to form their inverse via a Choleski
decomposition (Lines~44--49). The already centred sets \tt{p} and
\tt{q} are subsequently orthogonalised (Lines~51--56); so that \tt{p}
and \tt{q} are now solely related by a rotational matrix.


\subsection{Simulation}\label{m:sim}
...
Section~\ref{a:sim}, as well as in my Git repository at
...
\subsection{Projected 2d \gls{npc}}\label{m:2dsmlm}

As a second proof of work, we tested both rigid and affine recovery on
several real dual channel \gls{smlm} data sets of \gls{npc}s of two-color in focus
measurements of different cells successively recorded in the same
sample. The fluorescence images are analysed with \e{Thunderstorm}, a
\gls{dstorm} plugin for ImageJ, to obtain 2d localisations based on Gaussian
fits.
...
channels. The code is listed in Section~\ref{a:cockpit}, and
Section~\ref{a:ampel}, as well as in my Git repository at \t{git}.
...
\SI{1.5}{\NA} objective; for X, Y and Z axis (top, middle, and
bottom); for both red and blue channel (colors) is shown in
Figure~\ref{f:all}.\\
...
\clearpage\section{3d SMLM Analysis}\label{s:r:ana}

Applying the \gls{smlm} described in Section~\t{sec} for the stack of
...
As a proof of work we used \gls{npc}s as sort of a well defined biological
...
\e{how good are the \gls{npc}s resolved}. The Section \t{sec} is thus
dedicated to find some metric for \e{best resolved} \gls{npc}s.
...
\section{3d \gls{smlm} Analysis: \gls{npc}}\label{s:r:ananpc}

We use \gls{npc}s as a test target for our method, thus we have some
additional knowledge, like the geometry of each individual \gls{npc}: they
...
the known \gls{npc} radius (Lines~20--24). The quantity \tt{twofold} is
...
\clearpage\section{Dual Channel 3d \gls{smlm}}\label{r:dc}

In order to perform two-colour \gls{smlm} we marked \gls{npc} and bead samples
with an equal amount of both AF488 and AF647 fluorescence dyes.\\
...
dimensional random data sets in Section~\ref{r:3dsim}, as well as
based on two dimensional \gls{smlm} data sets in Section~\ref{r:2dsim}.


\subsection{Simulation}\label{r:3dsim}
...
\clearpage\subsection{Projected 2d NPC}\label{r:2dsim}

Lacking proper 3d dual channel \gls{smlm} data, we choose to use
...
\subsubsection{Rigid Projection}\label{r:dcnpcr}
...
\subsubsection{Affine Projection}\label{r:dcnpca}
...
\section{3d \gls{smlm} Analysis: \gls{npc}}~\label{s:d:ananpc}
...
\item [quality] The definitions of the two quality entities
  ...
  our known \gls{npc} geometry might work better, such as fitting the
...
\section{Dual Channel Projections}
...
$\b{t}$ and rotation matrices $\b{R}$ in Section~\ref{r:dcsim} enables
...
depths. This yields a full PSF model to be used for de-focus 3d \gls{smlm}.
...
\section{3d SMLM Analysis}


\section{3d SMLM Analysis: NPC}


\section{Dual Channel 3d SMLM}
...
dangerous caveat for the dual channel \gls{smlm} analysis, where we cannot
rule out rotations.
...
As a second proof of work, we tested both rigid and affine recovery on
several real dual channel \gls{smlm} data sets of two color in focus
measurements of different cells successively recorded in the same
sample.
...
\clearpage\section{Dual Channel 3d SMLM}\label{a:dc}


\subsection{Cockpit: cockpit.m}\label{a:cockpit}
...
\subsection{Splice Channels: ampel.m}\label{a:ampel}
...
\subsection{Rigid Transformation: rig.m}\label{a:rig}
...
\subsection{Affine Transformation: affine.m}\label{a:affine}
...
\subsection{Clustering: dbscan.m}\label{a:dbscan}
\begin{minted}{octave}
#!/bin/octave
...
\subsection{Simulate Dual Chanel 3d SMLM Data: simulate.m}\label{a:sim}
\begin{minted}{octave}
#!/bin/octave
## @ moritz siegel
